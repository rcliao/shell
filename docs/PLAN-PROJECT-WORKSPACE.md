...
- **Topic delivery without scheduler surgery**: inbound group-topic messages
  ...
  carries a threadID on every send. What's missing (re-review F4) is the
  scheduler's *delivery* callbacks — `NotifyFunc`/`PromptFunc` are
  chatID-only — so P2 plumbs a threadID through those callbacks and the
  bridge's scheduled-response send (delivery only; fires keep their existing
  (chat, 0) serialization key — the PartitionKey re-partitioning risk stays
  out of scope). Deterministic plumbing was chosen over agent-discretionary
  relay delivery, which would sit in write-verification territory.
...
**Autonomous research.** Schedule fires (payload names the project — no
classification involved) → ONE bounded research pass → doc edit + commit →
surgical block re-render in Notion → ≤3-line delta in chat with link → row
in pinned list updated (`last_research_at`).
...
**P2 — Doc core + project home v1 (~1.5 wk, re-costed for F4).**
Per-project git + template + doc-write path (receipts, dirty check) +
research schedule + threadID through scheduler delivery callbacks + delta
discipline + SendDocument/Pin/edit in transport (incl. a send-returning-
message-id method — `Notify` returns nothing today) + `type="document"` +
pinned 📋 Projects message + `/projects` + `shell project` CLI.
