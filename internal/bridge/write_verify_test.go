...
func TestClassifyWrite(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "verbal save — claims Notion write, no tool call (the mami bug)",
			userMsg:  "晚餐：地瓜 + 優格",
			response: "收到 mami 📝 6/20 晚餐記下，補進 Notion ✅",
			calls:    nil,
			want:     "verbal_save",
		},
		{
		...
		},
		{
			...
			userMsg:  "今天天氣如何？",
			response: "今天 San Dimas 晴天 ☀️ 26 度",
			calls:    nil,
			want:     "",
		},
		{
