...
The scheduler needs to accept a **target chat_id** that's different from `SHELL_CHAT_ID`. When running in heartbeat (chat_id=0), the agent should be able to say "schedule this for mami's chat (800000001)."

**Options:**
1. **`--chat` flag on shell-schedule**: `shell-schedule cron --chat 800000001 --expr "0 21 * * *" --message "Flonase!"`
